...
| TXT-015 | Failed scene initialization replaces loading with a short error message and a single Back to Scene Map action. Editing remains unavailable until startup succeeds. Returning to the scene map cleans up the old renderer; users can reopen the scene. Returning from fullscreen preview reveals the editor immediately without a page-loading overlay; it enters this failure state only if canvas restoration fails. Cancelled startup cannot update the replacement page; optional statistics-cache failures leave a ready editor usable. | `initializationRecovery.test.js`; `initializationDiagnostics.test.js`; `initializationRecovery.browser.mjs`; `tests/vnPreview/recovery.browser.mjs` |
...
| TXT-B018 | Inject a graphics initialization rejection after creating the renderer. The scene previously remained covered by “Loading scene…” with no recovery action. | Represent startup failure explicitly, preserve the editor mount behind an inert surface, and clean up on exit. Abort abandoned startup work and ignore late results. | TXT-015; `initializationRecovery.browser.mjs` checks repeated failures, reopening scenes, Back to Scene Map, late graphics/file results, and unchanged project revision in Chromium/WebKit. |
| TXT-B019 | Close fullscreen preview, pause a file read during editor restoration, reopen fullscreen, then release the old read. In WebKit, the old restoration moved fullscreen's canvas back into the editor. | Cancel the editor initialization/restoration session before mounting fullscreen preview, preserving the immediate editor return. | `tests/vnPreview/reopen.browser.mjs` checks repeated reopening, cancellation, retained fullscreen canvas ownership, and absence of missing-cache warnings in Chromium/WebKit. |
| TXT-B020 | Close fullscreen preview, pause a restoration file read, dispatch an ordinary editor redraw, then reopen preview. The redraw joined the cancelled read and showed “Failed to load some scene assets.” Reproduced in Chromium. | Tie direct and queued redraws to their owning editor session, including shared pending asset loads, and stop cancelled redraws before warnings or canvas updates. | `runtime.test.js`; `tests/vnPreview/reopen.browser.mjs` verifies no asset warning or stolen fullscreen canvas in Chromium/WebKit. |
| TXT-B021 | Publish project changes during graphics initialization and the final project-info read. Startup discarded the events and enabled editing with stale lines and selection. | Refresh the latest project snapshot synchronously before enabling editing, reconcile selection/drafts, and schedule a canvas refresh when the revision changed. | `initializationRecovery.test.js` publishes updates through the mounted project subscription at both async boundaries. |
| TXT-B022 | Type while the editor canvas restores after preview, then let restoration fail. Backups, navigation, and unmount skipped forced draft persistence. | Keep persistence barriers after any editor session has become usable, independently of later renderer failure. | `initializationRecovery.test.js` holds the draft write pending and verifies backup, navigation, and unmount wait for it. |
...
Scene initialization recovery is also checked against the existing watch server:

```bash
node tests/sceneEditor/initializationRecovery.browser.mjs
```
