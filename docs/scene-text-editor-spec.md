...
| TXT-002 | Typing, replacement input, soft breaks, paste, Cut, and deletion replace the entire selected range, including forward/backward selections across scene lines. A supplied event target range wins over a stale live caret. Preserve the first surviving line ID. | `lexicalNewlineInput.test.js` TXT-002; `lexicalEditingContracts.browser.mjs` TXT-002 |
