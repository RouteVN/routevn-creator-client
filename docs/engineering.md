...
name. The macOS default also copies the project icon. The Windows icon starts
empty, is required, and is uploaded through a 256-by-256 crop before the
platform can be created. The Web identifier starts blank, is required and
editable, and controls browser
save-data identity; older stored Web records are backfilled once from
`projectInfo.namespace`. The Windows and macOS identifiers start blank and are
both required and editable. Changing the macOS identifier changes the exported
app and save-data identity, so builds using different identifiers do not share
saves.
Preview records stored under the former
`releaseInfo.<platform>` keys are copied into the `platformDetails.<platform>`
namespace when read. A later project icon update fills an existing macOS
platform icon only while it remains empty; Windows always requires a separately
uploaded icon. The records otherwise have no cross-platform synchronization or
...
`manifest.webmanifest`, and the bundle instructions; Web export uses the Web
Application Name and project-owned icon. The same Application Name is used for
the manifest `name` and `short_name`. Web theme and startup background colors
use fixed runtime defaults. The icon is emitted as 192px and 512px PNGs. HTML
