export const createMobileAudioRuntime = ({
  ...
  let nativeActive = windowTarget.routeVNAppActive ?? true;
  let active = nativeActive && !documentTarget.hidden;
  ...
  const reportError = (error) => {
    console.error("[Mobile audio] Failed to update playback lifecycle", error);
  };
  ...
  const updateActivity = () => {
    ...
    active = nextActive;
    if (!active) {
      // Pause media outputs before suspending their producing contexts.
      for (const listener of activityListeners) {
        Promise.resolve(listener(false)).catch(reportError);
      }
    ...
    for (const listener of activityListeners) {
      Promise.resolve(listener(true)).catch(reportError);
    }
  ...
  const createAudioContext = () => {
    ...
    // Keep that request pending until the native app and document are both visible.
    context.resume = () => {
      if (!active) {
        entry.resumeOnForeground = true;
