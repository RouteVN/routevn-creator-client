import { base64ToUint8Array, callIOSBridge } from "./bridge.js";
import {
...
export const createIOSSqliteConnection = ({ dbPath }) => {
  if (!dbPath) {
    throw new Error("dbPath is required for iOS SQLite.");
  }
  ...
  return {
    async init() {
      await callIOSBridge("sqliteOpen", { dbPath });
    },
    ...
    async select(sql, args = []) {
      ...
      let resultSize;
      try {
        const rows = await callIOSBridge("sqliteQuery", {
          dbPath,
          sql,
        ...
        logAndroidBridgeTiming({
          method: "ios.sqlite.select.total",
    ...
    async execute(sql, args = []) {
      return callIOSBridge("sqliteExec", {
        dbPath,
        sql,
        args: normalizeArgs(args),
      });
    ...
    async close() {
      await callIOSBridge("sqliteClose", { dbPath });
    },
  };
};
