...
const resolveTextureFields = (texture) => {
  ...
  const firstItem = Array.isArray(texture?.items) ? texture.items[0] : undefined;
  if (firstItem?.src) {
...
export const createParticlePresetSelectionForm = () => ({
  ...
  description: "Pick a starting preset. The next step opens the full editor with those values prefilled.",
  fields: [
    {
      name: "presetId",
      type: "select",
...
export const buildParticleFormValues = ({
  ...
  return {
    ...
    customWidth: toTextValue(bounds.custom?.width ?? resolvedParticle.width ?? 1280),
...
const buildSourceDefinition = ({ values, width, height }) => {
  ...
  return {
    kind: "rect",
    data: {
      x: toOptionalNumber(values.sourceX) ?? Math.round((width - defaultWidth) / 2),
...
export const buildParticlePayload = ({
  ...
  const fallbackTimedDuration =
    typeof resolvedBaseDuration === "number" && Number.isFinite(resolvedBaseDuration)
  ...
  modules.emission = {
    ...
    duration:
      durationMode === "timed"
        ? Math.max(0, toNonNegativeNumber(values?.durationSeconds, fallbackTimedDuration))
        : "infinite",
    particleLifetime: {
...
export const buildParticleDetailFields = (input) => {
  ...
  return [
    ...
    },
    {
      type: "text",
      label: "Texture Image",
      value: summarizeTexture(item.modules?.appearance?.texture, imagesData?.items),
    },
    {
      type: "text",
      label: "Lifetime",
