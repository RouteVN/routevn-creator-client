import { assetItems, userInterfaceItems, systemConfigItems } from '../resourceTypes/resourceTypes.store.js';
...
export const toViewData = ({ state }) => {
  ...
  const findMatchingItem = () => {
    ...
    const categoryMappings = [
      { items: assetItems, sidebarPath: state.items.find(item => item.title === "Assets")?.path },
      { items: userInterfaceItems, sidebarPath: state.items.find(item => item.title === "User Interface")?.path },
      { items: systemConfigItems, sidebarPath: state.items.find(item => item.title === "System Config")?.path }
    ];
    ...
    for (const { items, sidebarPath } of categoryMappings) {
      ...
      const matchFound = items.some(item => {
        ...
        const currentResourceMatch = currentPath.match(/^\/project\/resources\/([^\/\?]+)/);
        ...
        const itemSingular = item.id.endsWith('s') ? item.id.slice(0, -1) : item.id;
    ...
    if (currentPath.startsWith('/project/settings')) {
      const settingsItem = state.items.find(item => item.title === "Settings");
