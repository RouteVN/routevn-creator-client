...
export const handleSplitLine = async (deps, payload) => {
  ...
  console.log(
    `[LE][${timestamp}] handleSplitLine called | lineId:`,
    lineId,
    "| lock:",
    splittingLineId,
    "| leftContent:",
    leftContent.substring(0, 20),
    "| rightContent:",
    rightContent.substring(0, 20),
  );
  ...
  console.log(
    `[LE][${scheduleTime}] Scheduling RAF to focus newLineId:`,
    newLineId,
    "| lock:",
    store.selectSplittingLineId(),
  );
  requestAnimationFrame(() => {
    const rafTime = performance.now().toFixed(2);
    console.log(
      `[LE][${rafTime}] RAF executing - focusing newLineId:`,
      newLineId,
      "| lock:",
      store.selectSplittingLineId(),
    );
    ...
    console.log(
      `[LE][${rafTime}] Scheduling nested RAF to clear lock | lock:`,
      store.selectSplittingLineId(),
    );
    requestAnimationFrame(() => {
      const nestedRafTime = performance.now().toFixed(2);
      console.log(
        `[LE][${nestedRafTime}]  Nested RAF executing - clearing lock | was:`,
        store.selectSplittingLineId(),
      );
