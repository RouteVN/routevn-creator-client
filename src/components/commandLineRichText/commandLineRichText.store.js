...
export const addFormatting = (state, { start, end, formatType }) => {
  ...
  let existingRange = state.formattedRanges.find(
  ...
  if (existingRange) {
    ...
    if (!hasActiveFormats) {
      state.formattedRanges = state.formattedRanges.filter(
        (r) => r !== existingRange,
      );
    }
  } else {
...
export const toViewData = ({ state, props }, payload) => {
  ...
  const content =
    props?.stepContent ||
    state.stepContent ||
    "No content available - props not working yet";

  // Generate formatted HTML content
  const formattedContent = generateFormattedHTML(
    content,
    state.formattedRanges,
  );
