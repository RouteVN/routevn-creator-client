...
DECLARED_RETTANGOLI_VERSION=$(node -p "require('./package.json').dependencies['@rettangoli/ui']" 2>/dev/null || echo "1.7.3")
RETTANGOLI_VERSION=""

if [ -f "${LOCAL_RETTANGOLI_PACKAGE_JSON}" ]; then
  RETTANGOLI_VERSION=$(node -p "require('./${LOCAL_RETTANGOLI_PACKAGE_JSON}').version" 2>/dev/null || true)
fi

if [ -z "${RETTANGOLI_VERSION}" ]; then
  RETTANGOLI_VERSION="${DECLARED_RETTANGOLI_VERSION#^}"
  RETTANGOLI_VERSION="${RETTANGOLI_VERSION#~}"
  RETTANGOLI_VERSION="${RETTANGOLI_VERSION#file:}"
fi
