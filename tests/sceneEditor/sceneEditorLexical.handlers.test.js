...
describe("sceneEditorLexical.handlers actions dialog", () => {
  ...
  it.each([false, true])(
    "focuses the created line unless navigation supersedes it (moved=%s)",
    async (moved) => {
      ...
      try {
        ...
        if (moved) state.selectedLineId = "line-1";
        while (animationFrame.callbacks.length) {
          animationFrame.callbacks.shift()();
        }
        expect(linesEditor.focusLine).toHaveBeenCalledTimes(moved ? 0 : 1);
        if (!moved) {
