...
describe("tauri collab client store locking", () => {
  ...
  it("loads committed and draft metadata without selecting event payloads", async () => {
    const fakeDb = {
      execute: vi.fn(async () => ({ rowsAffected: 0 })),
      select: vi.fn(async (sql) => {
        if (String(sql).includes("SELECT draft_clock, partition, type")) {
          return [
            {
              draft_clock: 4,
              partition: "m",
        ...
        if (String(sql).includes("SELECT committed_id, partition, type")) {
          return [
            {
              committed_id: 11,
              partition: "m",
    ...
    const store = await createPersistedTauriProjectStore({
      projectPath: "/projects/metadata",
      projectId: "project-metadata",
    });

    await expect(
      store.listCommittedMetadataAfter({
        sinceCommittedId: 10,
        limit: 32,
      }),
    ).resolves.toEqual([
    ...
    await expect(store.listDraftMetadataOrdered()).resolves.toEqual([
      {
        draftClock: 4,
        partition: "m",
        type: "image.create",
    ...
    const [metadataSql, metadataArgs] = fakeDb.select.mock.calls.find(([sql]) =>
      String(sql).includes("SELECT committed_id, partition, type"),
    );
    const [draftMetadataSql] = fakeDb.select.mock.calls.find(([sql]) =>
      String(sql).includes("SELECT draft_clock, partition, type"),
    );
    expect(metadataSql).not.toContain("payload");
    expect(metadataArgs).toEqual([10, 32]);
    expect(draftMetadataSql).not.toContain("payload");
